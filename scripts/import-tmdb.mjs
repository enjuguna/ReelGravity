...
const seedIds = [550, 11, 13, 155, 157336, 19995, 920, 862, 508442, 354912, 129, 207703, 497, 680, 603, 278, 694, 597, 120467, 603692, 372058, 438631, 389, 284054, 634649];
...
const discovered = [];
for (let page = 1; page <= 8 && discovered.length < 30; page += 1) {
...
const ids = [...new Set([...seedIds, ...discovered])].slice(0, 30);
...
if (movies.length !== 30 || new Set(movies.map(movie => movie.id)).size !== 30) throw new Error(`Expected 30 unique movies, received ${movies.length}.`);
